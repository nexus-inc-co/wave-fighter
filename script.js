...
class Tank extends Character {
  constructor(x, y) {
    super(x, y);
    this.width = 100; // Tank-specific dimensions
    this.height = 120;
    this.speed = 4; // Tank is slower
    this.image.src = "../Fotos/sprites/character_sprites.png";
    this.frameWidth = 500; // Tank-specific sprite frame size
...
class Enemy {
  ...
  attack(player) {
    if (this.checkCollision(player)) {
      player.health -= 5; // Damage the player on attack
